...
class AuthPageController(private val authService: AuthService) {
    ...
    fun login(model: Model): String {
        if (!model.containsAttribute("form")) model.addAttribute("form", PhoneForm())
        return "auth/phone"
    }
    ...
    fun requestCode(@Valid @ModelAttribute("form") form: PhoneForm, bindingResult: BindingResult, model: Model): String {
        ...
        model.addAttribute("form", CodeForm(form.phone))
    ...
    fun code(@RequestParam phone: String, model: Model): String {
        model.addAttribute("phone", phone)
        model.addAttribute("form", CodeForm(phone))
...
data class PhoneForm(@field:Pattern(regexp = "^\\+?[0-9]{10,15}$", message = "Введите телефон в международном формате") var phone: String = "")
data class CodeForm(
